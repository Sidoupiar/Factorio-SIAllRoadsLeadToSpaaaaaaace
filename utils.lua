...
---@param ConstantsDataPrefix string|nil -- ConstantsData 在代码的各种位置中进行注册时使用的名称前缀 , 防止重名 , 不影响注册的原型数据 , 默认 "SIConstants_"
---@param CodeNamePrefix string|nil -- ConstantsData 在代码的各种位置中进行显示时使用的名称前缀 , 防止重名 , 影响注册的原型数据的 name 属性 , 默认 "SI"
---@param ShowNamePrefix string|nil -- ConstantsData 在代码的各种位置中进行显示时使用的名称前缀 , 防止重名 , 影响注册的原型数据和本地化字符串 , 生成原型数据名称时此前缀也会被包含进去 , 默认 "SI"
---@return SIInit -- 自身
								...
								if autoload.Groups then
									...
									for groupID , groupData in pairs( autoload.Groups ) do
										...
										local group =
										{
											...
											icon = constantsData.PicturePath .. groupCodeName .. SICommon.CodeNameSuffix.ICON .. ".png" ,

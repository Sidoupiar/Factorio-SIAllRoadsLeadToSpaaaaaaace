-- ======================================================================<br>
-- 虽然你可以在这里删除任意的功能模块以便移除它的功能<br>
-- 但是 Core 功能模块包不可以移除 , 因为它包含了一些公共内容 , 移除后会导致整个 MOD 不可用<br>
-- ======================================================================<br>
-- 注意 :<br>
-- 如果在一个存档中反复启用和禁用某一个或某几个功能模块 , 可能会出现意想不到的 bug<br>
-- ======================================================================<br>
-- 编号规划 :<br>
-- 0000 - 0099 = 游戏内工具性质的内容<br>
-- 0100 - 0199 = 对原版的调整<br>
-- 0200 - 0299 = 非独立的扩展系统 , 主要用作各种逻辑的统一前置存在<br>
-- 1000 - 1099 = 标准生产流程 , 工业类型 , 包括采矿和零件生产<br>
-- 1100 - 1199 = 标准生产流程 , 农业类型 , 包括食品和加工<br>
-- 1200 - 1299 = 标准生产流程 , 特殊类型<br>
-- 2000 - 2099 = 扩展系统<br>
-- 3000 - 3099 = 流程外扩展 , 不影响游戏进度的存在<br>
-- ======================================================================<br>
local PackageConfig =
{
	-- ======================================================================<br>
	-- Core 模块不可删除不可禁用 , 它是所有模块的基础<br>
	-- ======================================================================<br>
	{
		-- ======================================================================<br>
		-- 功能模块所在的文件夹名称<br>
		-- ======================================================================<br>
		PackageName = "0000_Core" ,

		-- ======================================================================<br>
		-- 默认情况下是否启用这个功能模块 , 默认 : false<br>
		-- ======================================================================<br>
		Enabled = true ,

		-- ======================================================================<br>
		-- 是否强制启用这个模块 , 默认 : false<br>
		-- 设置为 true 后 , Enabled 属性和 MOD 设置中的 [启用功能模块] 设置会失去效果 , 仅能用作说明展示<br>
		-- ======================================================================<br>
		ForceEnabled = true ,

		-- ======================================================================<br>
		-- 强制依赖<br>
		-- ======================================================================<br>
		-- 由于每个模块都是可以删除的 , 因此会出现删除部分模块后出现无法处理的异常<br>
		-- 强制依赖会阻止这种情况发生<br>
		-- 强制依赖的模块未加载时 , 当前模块加载时会强制弹出加载错误<br>
		-- 因为是依赖 , 所以强制依赖的模块必须在当前模块之前加载 , 即在此列表中更加靠前的位置<br>
		-- 强制依赖使用 PackageName 而非 ConstantsData.CodeName<br>
		-- ======================================================================<br>
		Requires =
		{
			-- Core 模块没有依赖
		} ,

		-- ======================================================================<br>
		-- 互斥依赖<br>
		-- ======================================================================<br>
		-- 部分模块会因为功能相似性而没必要同时启用 , 因此增加了互斥依赖功能<br>
		-- 如果互斥依赖的模块已经加载了 , 则当前模块无法加载<br>
		-- 和强制依赖一样 , 互斥依赖也只能排除在此列表中更加靠前的位置的模块<br>
		-- ======================================================================<br>
		Excepts =
		{
			-- Core 模块没有排除
		} ,

		-- ======================================================================<br>
		-- 以下是一些配置项<br>
		-- 它们会影响相应部分的 MOD 内容效果 , 具体效果参见具体属性的注释和说明<br>
		-- 只要是合法的值 , 就不会有任何副作用<br>
		-- ======================================================================<br>
		-- 它们和玩家设置 SISettings 不同<br>
		-- 这些配置仅供代码设置和使用 , 玩家无法直接修改这些配置<br>
		-- 这些配置只能用于 data 阶段和 control 阶段 , 且创建完毕对应功能模块的 ConstantsData 之后才会有效 ( 即调用完 ConstantsData.AfterLoad 之后 )<br>
		-- ======================================================================<br>
		-- 对于 data 阶段的配置 , 除了直接修改配置项的值以外 , 还可以使用其他 MOD 修改它们的值<br>
		-- 位置 : SIConfigs.[ ConstantsData.ID ].[ 配置项 ]<br>
		-- ======================================================================<br>
		Configs =
		{
			-- ======================================================================<br>
			-- 此项只是样例 , 并没有实际效果<br>
			-- ======================================================================<br>
			-- 表中的每个值都可以单独修改 , 子表中的也是<br>
			-- 一些情况下 , 除了直接修改这些值以外 , 还可以在其他 MOD 的 data.lua ( 阶段 ) 中使用代码来修改它们的值 , 甚至有些还可以在 control.lua 的 on_init 和 on_load 阶段中使用 remote 接口来修改它们的值<br>
			-- ======================================================================<br>
		}
	} ,
	-- ======================================================================<br>
	-- 添加了粉图黄图紫图<br>
	-- ======================================================================<br>
	{
		PackageName = "0001_ToolMap" ,
		Requires =
		{
			"0000_Core"
		} ,
		Configs = {}
	} ,
	-- ======================================================================<br>
	-- 添加了一套调试使用的高性能物品<br>
	-- ======================================================================<br>
	{
		PackageName = "0002_DebugItem" ,
		Requires =
		{
			"0000_Core"
		} ,
		Configs =
		{
			-- ======================================================================<br>
			-- 调试使用的实体的血量<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			DebugEntityHealthMult = 1.0 ,

			-- ======================================================================<br>
			-- 修改作为调试工具的坦克 [超越之安如磐石战车] 的实际大小<br>
			-- 包括视觉上的体积和实际的碰撞体积<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			TankSize = 8.0
		}
	} ,
	-- ======================================================================<br>
	-- 添加了自动填充功能<br>
	-- ======================================================================<br>
	{
		PackageName = "0003_AutoInsert" ,
		Enabled = true ,
		Requires =
		{
			"0000_Core"
		} ,
		Configs = {}
	} ,
	-- ======================================================================<br>
	-- 添加了快速开局功能<br>
	-- ======================================================================<br>
	{
		PackageName = "0004_QuickStart" ,
		Requires =
		{
			"0000_Core"
		} ,
		Configs =
		{
			-- ======================================================================<br>
			-- 是否每局游戏只赠送一次 , 用于多人游戏<br>
			-- 禁用后 , 每次新玩家进入这局游戏都会赠送一次大礼包<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			SendOnce = true ,

			-- ======================================================================<br>
			-- 遇到不存在的物品是否弹出报错信息 , 主要用于排除代码错误<br>
			-- 形式是游戏内的报错信息 , 不是弹窗<br>
			-- 关闭此项后 , 遇到不存在的物品时会直接跳过这个物品 , 主要用于多 MOD 兼容<br>
			-- ======================================================================<br>
			-- 除了直接修改以外<br>
			-- 也可以在其他 MOD 的 control.lua 的 on_init 和 on_load 阶段中<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Set" , "SIQuickStart" , "ErrorWhenItemNotExist" , 值 ) ] 来修改它的值 , 每次重启游戏都需要设置<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Get" , "SIQuickStart" , "ErrorWhenItemNotExist" ) ] 来读取它的值<br>
			-- ======================================================================<br>
			ErrorWhenItemNotExist = true ,

			-- ======================================================================<br>
			-- 开局时获得的物品列表<br>
			-- 结构是数组 , 填写要获得的物品名称 , 数量和品质<br>
			-- 设置为 nil 则跳过逻辑<br>
			-- ======================================================================<br>
			-- 除了直接修改以外<br>
			-- 也可以在其他 MOD 的 control.lua 的 on_init 和 on_load 阶段中<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Set" , "SIQuickStart" , "StartUp_ItemList" , 值 ) ] 来修改它的值 , 每次重启游戏都需要设置<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Get" , "SIQuickStart" , "StartUp_ItemList" ) ] 来读取它的值<br>
			-- ======================================================================<br>
			StartUp_ItemList =
			{
				-- 填写物品名称 , 数量和品质
				{ Name = "iron-plate"   , Count = 1000 , Quality = nil } ,
				{ Name = "copper-plate" , Count = 1000 , Quality = nil }
			} ,

			-- ======================================================================<br>
			-- 放置物品的箱子的名称列表 , 创建箱子时会随机选择列表中的箱子<br>
			-- 结构是数组 , 填写箱子名称 , 随机权重和品质 , 为了保证稳定性 , 最好填写 1 x 1 大小的箱子<br>
			-- 默认是普通品质的黄箱<br>
			-- 设置为 nil , 没有箱子 , 或权重为 0 则无法赠送大礼包<br>
			-- ======================================================================<br>
			-- 除了直接修改以外<br>
			-- 也可以在其他 MOD 的 control.lua 的 on_init 和 on_load 阶段中<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Set" , "SIQuickStart" , "StartUp_ChestList" , 值 ) ] 来修改它的值 , 每次重启游戏都需要设置<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Get" , "SIQuickStart" , "StartUp_ChestList" ) ] 来读取它的值<br>
			-- ======================================================================<br>
			StartUp_ChestList =
			{
				-- 填写箱子名称 , 随机权重和品质
				{ Name = "logistic-chest-storage" , RandomWeight = 1000 , Quality = nil }
			} ,

			-- ======================================================================<br>
			-- 一个箱子是否只放一种物品<br>
			-- ======================================================================<br>
			-- 除了直接修改以外<br>
			-- 也可以在其他 MOD 的 control.lua 的 on_init 和 on_load 阶段中<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Set" , "SIQuickStart" , "StartUp_OneChestOneItem" , 值 ) ] 来修改它的值 , 每次重启游戏都需要设置<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Get" , "SIQuickStart" , "StartUp_OneChestOneItem" ) ] 来读取它的值<br>
			-- ======================================================================<br>
			StartUp_OneChestOneItem = false ,

			-- ======================================================================<br>
			-- 箱子如果是黄箱的话 , 是否自动设置物品筛选<br>
			-- 需要启用 OneChectOneItem 项<br>
			-- ======================================================================<br>
			-- 除了直接修改以外<br>
			-- 也可以在其他 MOD 的 control.lua 的 on_init 和 on_load 阶段中<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Set" , "SIQuickStart" , "StartUp_AutoChestFilter" , 值 ) ] 来修改它的值 , 每次重启游戏都需要设置<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Get" , "SIQuickStart" , "StartUp_AutoChestFilter" ) ] 来读取它的值<br>
			-- ======================================================================<br>
			StartUp_AutoChestFilter = false ,

			-- ======================================================================<br>
			-- 生成箱子的时候会给玩家控制的角色留下通行的道路 , 此项规定道路的间距 , 取值范围 : 2 ~ 32 , 单位 : 格<br>
			-- 它的默认值是 8 格 , 设置为 nil 会使用默认值<br>
			-- ======================================================================<br>
			-- 除了直接修改以外<br>
			-- 也可以在其他 MOD 的 control.lua 的 on_init 和 on_load 阶段中<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Set" , "SIQuickStart" , "StartUp_ChestSpaceSize" , 值 ) ] 来修改它的值 , 每次重启游戏都需要设置<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Get" , "SIQuickStart" , "StartUp_ChestSpaceSize" ) ] 来读取它的值<br>
			-- ======================================================================<br>
			StartUp_ChestSpaceSize = 8
		}
	} ,
	-- ======================================================================<br>
	-- 添加了权限管理功能<br>
	-- ======================================================================<br>
	{
		PackageName = "0005_Permission" ,
		Requires =
		{
			"0000_Core"
		} ,
		Configs = {}
	} ,
	-- ======================================================================<br>
	-- 添加了虚空存储逻辑功能<br>
	-- ======================================================================<br>
	{
		PackageName = "0010_VoidStorage" ,
		Requires =
		{
			"0000_Core"
		} ,
		Configs = {}
	} ,
	-- ======================================================================<br>
	-- 添加了触发解锁逻辑功能<br>
	-- ======================================================================<br>
	{
		PackageName = "0011_Unlocker" ,
		Requires =
		{
			"0000_Core"
		} ,
		Configs = {}
	} ,
	-- ======================================================================<br>
	-- 添加了建造限制逻辑功能<br>
	-- ======================================================================<br>
	{
		PackageName = "0012_BuildLimit" ,
		Requires =
		{
			"0000_Core"
		} ,
		Configs = {}
	} ,
	-- ======================================================================<br>
	-- 添加了一些修改各种物品属性的功能<br>
	-- ======================================================================<br>
	{
		PackageName = "0100_ItemPropertyModify" ,
		Requires =
		{
			"0000_Core"
		} ,
		Configs =
		{
			-- ======================================================================<br>
			-- 修改物品属性<br>
			-- 以下是各项设置的默认值<br>
			-- ======================================================================<br>

			-- ======================================================================<br>
			-- 维修工具耐久度倍率<br>
			-- 取值范围 0 至 10 0000 0000<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			RepairToolDurabilityMultiplier = 1.0 ,

			-- ======================================================================<br>
			-- 维修工具维修速度倍率<br>
			-- 取值范围 0 至 10 0000 0000<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			RepairToolSpeedMultiplier = 1.0 ,

			-- ======================================================================<br>
			-- 工具 ( 科技包 ) 耐久度倍率<br>
			-- 取值范围 0 至 10 0000 0000<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			ToolDurabilityMultiplier = 1.0 ,

			-- ======================================================================<br>
			-- 研究球速度倍率<br>
			-- 取值范围 0 至 10 0000 0000<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			LabSpeedMultiplier = 1.0 ,

			-- ======================================================================<br>
			-- 子弹弹夹数量倍率<br>
			-- 取值范围 0 至 10 0000 0000<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			MagazineAmmoSizeMultiplier = 1.0 ,

			-- ======================================================================<br>
			-- 电路接线距离倍率<br>
			-- 取值范围 0 至 10 0000 0000<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			CircuitDistanceMultiplier = 1.0 ,

			-- ======================================================================<br>
			-- 地下传送带距离倍率<br>
			-- 取值范围 0 至 10 0000 0000<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			UndergrundBeltDistanceMultiplier = 1.0 ,

			-- ======================================================================<br>
			-- 地下管道距离倍率<br>
			-- 取值范围 0 至 10 0000 0000<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			UndergrundPipeDistanceMultiplier = 1.0
		}
	} ,
	-- ======================================================================<br>
	-- 添加了修改物品堆叠的功能<br>
	-- ======================================================================<br>
	{
		PackageName = "0101_ItemStackSizeModify" ,
		Requires =
		{
			"0000_Core"
		} ,
		Configs =
		{
			-- ======================================================================<br>
			-- 全局基础修改<br>
			-- 以下是各项设置的默认值<br>
			-- ======================================================================<br>

			-- ======================================================================<br>
			-- 是否忽略建议禁止堆叠的物品<br>
			-- 如果启用的话 , 一些不应该堆叠的物品也会被增加堆叠 , 比如武器 , 护甲等不建议堆叠的物品也会变得可以堆起来 , 它可能会造成其他的问题 , 需要慎重开启<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			IgnoreCannotStack = false ,

			-- ======================================================================<br>
			-- 倍率修改堆叠数量<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			ItemStackSizeMultiplier =
			{
				-- ======================================================================<br>
				-- 允许的物品堆叠数量最小值 , 任意物品堆叠数量小于此值时 , 会强制变为此值<br>
				-- 如果为 0 则禁用限制<br>
				-- ======================================================================<br>
				-- 只能直接修改<br>
				-- 此项需要从代码里直接修改 , 不生成游戏内设置项目<br>
				-- ======================================================================<br>
				Min = 0 ,

				-- ======================================================================<br>
				-- 允许的物品堆叠数量最大值 , 任意物品堆叠数量大于此值时 , 会强制变为此值<br>
				-- 如果为 0 则禁用限制<br>
				-- ======================================================================<br>
				-- 只能直接修改<br>
				-- 此项需要从代码里直接修改 , 不生成游戏内设置项目<br>
				-- ======================================================================<br>
				Max = 0 ,

				-- ======================================================================<br>
				-- 默认的物品堆叠数量的倍率 , 所有物品的堆叠数量都会乘以此值<br>
				-- 主要应用于下面 TypeList 各项的默认值<br>
				-- 如果为 0 则禁用逻辑<br>
				-- 取值范围 0 至 10 0000 0000<br>
				-- ======================================================================<br>
				-- 只能直接修改<br>
				-- 此项需要从代码里直接修改 , 不生成游戏内设置项目<br>
				-- ======================================================================<br>
				Multiplier = 1.0 ,

				-- ======================================================================<br>
				-- 指定类别的物品堆叠数量的倍率 , 该类别的所有物品的堆叠数量都会乘以此值<br>
				-- 如果为 0 则使用 Multiplier 作为默认值 , 如果是 nil 则忽略这个类别的物品<br>
				-- 取值范围 0 至 10 0000 0000<br>
				-- 键的可用值请见 SICommon.Types.StackableItems 的定义<br>
				-- ======================================================================<br>
				-- 只能直接修改<br>
				-- ======================================================================<br>
				TypeList =
				{
					[SICommon.Types.StackableItems.Item]        = 0.0 ,
					[SICommon.Types.StackableItems.ItemEntity]  = 0.0 ,
					[SICommon.Types.StackableItems.Tool]        = 0.0 ,
					[SICommon.Types.StackableItems.ToolRepair]  = 0.0 ,
					[SICommon.Types.StackableItems.RailPlanner] = 0.0 ,
					[SICommon.Types.StackableItems.Ammo]        = 0.0 ,
					[SICommon.Types.StackableItems.Capsule]     = 0.0 ,
					[SICommon.Types.StackableItems.Gun]         = 0.0 ,
					[SICommon.Types.StackableItems.Module]      = 0.0
				}
			} ,

			-- ======================================================================<br>
			-- 是否使用单独修改堆叠数量的逻辑<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- 此项需要从代码里直接修改 , 不生成游戏内设置项目<br>
			-- ======================================================================<br>
			EnableItemStackSizeList = false ,

			-- ======================================================================<br>
			-- 使用单独修改堆叠数量的逻辑时 , 遇到不存在的物品是否弹出报错信息 , 主要用于排除代码错误<br>
			-- 关闭此项后 , 遇到不存在的物品时会直接跳过这个物品 , 主要用于多 MOD 兼容<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- 此项需要从代码里直接修改 , 不生成游戏内设置项目<br>
			-- ======================================================================<br>
			ErrorWhenItemNotExist = true ,

			-- ======================================================================<br>
			-- 单独修改堆叠数量<br>
			-- 结构是键值对 , 键是物品的类型 , 可用值请见 SICommon.Types.StackableItems 的定义<br>
			-- 值是物品的 [ name ] 属性值与目标堆叠数量组成的子表<br>
			-- 最终 , 目标堆叠数量会覆盖原始堆叠数量<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- 此项需要从代码里直接修改 , 不生成游戏内设置项目<br>
			-- ======================================================================<br>
			ItemStackSizeList =
			{
				[SICommon.Types.StackableItems.Item] =
				{
					-- 资源
					["wood"]                             = 10000 , -- 木头
					["iron-ore"]                         = 10000 , -- 铁矿
					["copper-ore"]                       = 10000 , -- 铜矿
					["uranium-ore"]                      = 10000 , -- 铀矿
					["stone"]                            = 10000 , -- 石矿
					["coal"]                             = 10000 , -- 煤矿
					["uranium-235"]                      = 10000 , -- U235
					["uranium-238"]                      = 10000 , -- U238
					-- 冶炼物
					["iron-plate"]                       = 1000  , -- 铁板
					["steel-plate"]                      = 1000  , -- 钢板
					["copper-plate"]                     = 1000  , -- 铜板
					["plastic-bar"]                      = 1000  , -- 塑料
					-- 地板
					["stone-brick"]                      = 2000  , -- 石砖
					["concrete"]                         = 2000  , -- 混凝土
					["hazard-concrete"]                  = 2000  , -- 警示混凝土
					["refined-concrete"]                 = 2000  , -- 钢筋混凝土
					["refined-hazard-concrete"]          = 2000  , -- 警示钢筋混凝土
					["landfill"]                         = 2000  , -- 填海料
					-- 加工产物
					["iron-stick"]                       = 1000  , -- 铁棒
					["iron-gear-wheel"]                  = 500   , -- 铁齿轮
					["copper-cable"]                     = 1000  , -- 铜丝
					["red-wire"]                         = 500   , -- 红电路线
					["green-wire"]                       = 500   , -- 绿电路线
					["electronic-circuit"]               = 100   , -- 绿板
					["advanced-circuit"]                 = 100   , -- 红板
					["processing-unit"]                  = 100   , -- 蓝板
					["engine-unit"]                      = 100   , -- 内燃机
					["electric-engine-unit"]             = 100   , -- 电动机
					["solid-fuel"]                       = 400   , -- 固体燃料
					["rocket-fuel"]                      = 100   , -- 火箭燃料
					["nuclear-fuel"]                     = 100   , -- 核燃料
					["uranium-fuel-cell"]                = 100   , -- 核燃料罐
					["used-up-uranium-fuel-cell"]        = 100   , -- 用完的核燃料罐
					["sulfur"]                           = 500   , -- 硫磺
					["explosives"]                       = 500   , -- 炸药
					["battery"]                          = 500   , -- 电池
					["flying-robot-frame"]               = 100   , -- 机器人框架
					["low-density-structure"]            = 50    , -- 轻质框架
					["rocket-control-unit"]              = 50    , -- 火箭框架
					["satellite"]                        = 1     , -- 人造卫星
					-- 存储
					["wooden-chest"]                     = 100   , -- 木箱
					["iron-chest"]                       = 100   , -- 铁箱
					["steel-chest"]                      = 100   , -- 钢箱
					["logistic-chest-passive-provider"]  = 100   , -- 红箱
					["logistic-chest-storage"]           = 100   , -- 黄箱
					["logistic-chest-buffer"]            = 100   , -- 绿箱
					["logistic-chest-requester"]         = 100   , -- 蓝箱
					["logistic-chest-active-provider"]   = 100   , -- 紫箱
					["empty-barrel"]                     = 100   , -- 空桶
					-- 机器设备
					["burner-mining-drill"]              = 20    , -- 热力矿机
					["electric-mining-drill"]            = 20    , -- 电力矿机
					["pumpjack"]                         = 20    , -- 磕头机
					["offshore-pump"]                    = 100   , -- 水泵
					["pipe"]                             = 100   , -- 管道
					["pipe-to-ground"]                   = 100   , -- 地下管道
					["storage-tank"]                     = 100   , -- 水罐
					["pump"]                             = 100   , -- 管道泵
					["heat-pipe"]                        = 100   , -- 热管
					["stone-furnace"]                    = 100   , -- 石炉
					["steel-furnace"]                    = 100   , -- 钢炉
					["electric-furnace"]                 = 100   , -- 电炉
					["burner-inserter"]                  = 100   , -- 黑爪
					["inserter"]                         = 100   , -- 黄爪
					["fast-inserter"]                    = 100   , -- 蓝爪
					["filter-inserter"]                  = 100   , -- 紫爪
					["long-handed-inserter"]             = 100   , -- 红爪
					["stack-inserter"]                   = 100   , -- 绿爪
					["stack-filter-inserter"]            = 100   , -- 白爪
					["boiler"]                           = 100   , -- 锅炉
					["steam-engine"]                     = 20    , -- 蒸汽机
					["steam-turbine"]                    = 20    , -- 蒸汽轮机
					["solar-panel"]                      = 100   , -- 太阳能板
					["accumulator"]                      = 100   , -- 蓄电器
					["nuclear-reactor"]                  = 1     , -- 核反应堆
					["heat-exchanger"]                   = 100   , -- 热量交换机
					["small-electric-pole"]              = 100   , -- 小电线杆
					["medium-electric-pole"]             = 100   , -- 中电线杆
					["big-electric-pole"]                = 100   , -- 大电线杆
					["substation"]                       = 100   , -- 广电线杆
					["small-lamp"]                       = 100   , -- 灯泡
					["assembling-machine-1"]             = 100   , -- 组装机 mk1
					["assembling-machine-2"]             = 100   , -- 组装机 mk2
					["assembling-machine-3"]             = 100   , -- 组装机 mk3
					["oil-refinery"]                     = 100   , -- 炼油厂
					["chemical-plant"]                   = 100   , -- 化工厂
					["centrifuge"]                       = 100   , -- 离心机
					["lab"]                              = 100   , -- 研究球
					["train-stop"]                       = 100   , -- 火车站
					["rail-signal"]                      = 100   , -- 铁路普通信号灯
					["rail-chain-signal"]                = 100   , -- 铁路连锁信号灯
					["transport-belt"]                   = 100   , -- 黄带
					["fast-transport-belt"]              = 100   , -- 红带
					["express-transport-belt"]           = 100   , -- 蓝带
					["underground-belt"]                 = 100   , -- 地下黄带
					["fast-underground-belt"]            = 100   , -- 地下红带
					["express-underground-belt"]         = 100   , -- 地下蓝带
					["splitter"]                         = 100   , -- 黄筛选器
					["fast-splitter"]                    = 100   , -- 红筛选器
					["express-splitter"]                 = 100   , -- 蓝筛选器
					["roboport"]                         = 10    , -- 指令平台
					["beacon"]                           = 100   , -- 插件塔
					["rocket-silo"]                      = 1     , -- 火箭发射井
					["arithmetic-combinator"]            = 100   , -- 运算器
					["decider-combinator"]               = 100   , -- 比较器
					["constant-combinator"]              = 100   , -- 常量器
					["programmable-speaker"]             = 100   , -- 播音器
					["power-switch"]                     = 100   , -- 电闸
					-- 防御工事和炮塔
					["stone-wall"]                       = 100   , -- 围墙
					["gate"]                             = 100   , -- 闸门
					["radar"]                            = 10    , -- 雷达
					["land-mine"]                        = 10    , -- 地雷
					["gun-turret"]                       = 10    , -- 机枪炮塔
					["laser-turret"]                     = 10    , -- 激光炮塔
					["flamethrower-turret"]              = 10    , -- 喷火炮塔
					["artillery-turret"]                 = 1     , -- 重炮炮塔
					-- 模块装甲模块
					["solar-panel-equipment"]            = 1     , -- 太阳能模块
					["fusion-reactor-equipment"]         = 1     , -- 核能模块
					["battery-equipment"]                = 1     , -- 电池模块 mk1
					["battery-mk2-equipment"]            = 1     , -- 电池模块 mk2
					["night-vision-equipment"]           = 1     , -- 夜视模块
					["belt-immunity-equipment"]          = 1     , -- 锚定模块
					["personal-roboport-equipment"]      = 1     , -- 指令平台模块 mk1
					["personal-roboport-mk2-equipment"]  = 1     , -- 指令平台模块 mk2
					["exoskeleton-equipment"]            = 1     , -- 外骨骼模块
					["personal-laser-defense-equipment"] = 1     , -- 激光防御模块
					["discharge-defense-equipment"]      = 1     , -- 放电防御模块
					["energy-shield-equipment"]          = 1     , -- 能量护盾模块 mk1
					["energy-shield-mk2-equipment"]      = 1     , -- 能量护盾模块 mk2
					-- 机器人
					["construction-robot"]               = 100   , -- 建设机器人
					["logistic-robot"]                   = 100     -- 物流机器人
				} ,
				[SICommon.Types.StackableItems.ItemEntity] =
				{
					-- 载具
					["locomotive"]                       = 1     , -- 内燃机车
					["cargo-wagon"]                      = 1     , -- 货运车厢
					["fluid-wagon"]                      = 1     , -- 液罐车厢
					["artillery-wagon"]                  = 1       -- 重炮车厢
				} ,
				[SICommon.Types.StackableItems.Tool] =
				{
					-- 科技包
					["automation-science-pack"]          = 1000  , -- 红瓶
					["logistic-science-pack"]            = 1000  , -- 绿瓶
					["military-science-pack"]            = 1000  , -- 灰瓶
					["chemical-science-pack"]            = 1000  , -- 蓝瓶
					["production-science-pack"]          = 1000  , -- 紫瓶
					["utility-science-pack"]             = 1000  , -- 金瓶
					["space-science-pack"]               = 1000    -- 白瓶
				} ,
				[SICommon.Types.StackableItems.ToolRepair] =
				{
					-- 维修工具
					["repair-pack"]                      = 100     -- 维修包
				} ,
				[SICommon.Types.StackableItems.RailPlanner] =
				{
					-- 铁轨
					["rail"]                             = 500     -- 铁轨布设器
				} ,
				[SICommon.Types.StackableItems.Ammo] =
				{
					-- 弹药
					["firearm-magazine"]                 = 100   , -- 普通弹夹
					["piercing-rounds-magazine"]         = 100   , -- 穿甲弹夹
					["uranium-rounds-magazine"]          = 100   , -- 贫铀弹夹
					["shotgun-shell"]                    = 100   , -- 普通霰弹
					["piercing-shotgun-shell"]           = 100   , -- 穿甲霰弹
					["rocket"]                           = 100   , -- 火箭弹
					["explosive-rocket"]                 = 100   , -- 爆破火箭弹
					["atomic-bomb"]                      = 100   , -- 核能火箭弹
					["cannon-shell"]                     = 100   , -- 坦克炮弹
					["explosive-cannon-shell"]           = 100   , -- 爆破坦克炮弹
					["uranium-cannon-shell"]             = 100   , -- 贫铀坦克炮弹
					["explosive-uranium-cannon-shell"]   = 100   , -- 贫铀爆破坦克炮弹
					["flamethrower-ammo"]                = 100   , -- 喷火燃料罐
					["artillery-shell"]                  = 30      -- 重炮炮弹
				} ,
				[SICommon.Types.StackableItems.Capsule] =
				{
					-- 资源
					["raw-fish"]                         = 100   , -- 鲜鱼
					-- 投掷武器
					["grenade"]                          = 100   , -- 手雷
					["cluster-grenade"]                  = 100   , -- 破片手雷
					["destroyer-capsule"]                = 100   , -- 攻击胶囊
					["defender-capsule"]                 = 100   , -- 防御胶囊
					["poison-capsule"]                   = 100   , -- 毒素胶囊
					["slowdown-capsule"]                 = 100   , -- 减速胶囊
					["distractor-capsule"]               = 100   , -- 放电胶囊
					["cliff-explosives"]                 = 5     , -- 悬崖炸药
					["discharge-defense-remote"]         = 1     , -- 放电防御模块控制器
					["artillery-targeting-remote"]       = 1       -- 重炮控制器
				} ,
				[SICommon.Types.StackableItems.Gun] =
				{
					-- 武器
					["pistol"]                           = 1     , -- 手枪
					["submachine-gun"]                   = 1     , -- 机枪
					["shotgun"]                          = 1     , -- 霰弹枪
					["combat-shotgun"]                   = 1     , -- 战斗霰弹枪
					["rocket-launcher"]                  = 1     , -- 火箭筒
					["flamethrower"]                     = 1       -- 喷火器
				} ,
				[SICommon.Types.StackableItems.Module] =
				{
					-- 插件
					["speed-module"]                     = 100   , -- 速度插件 mk1
					["speed-module-2"]                   = 100   , -- 速度插件 mk2
					["speed-module-3"]                   = 100   , -- 速度插件 mk3
					["effectivity-module"]               = 100   , -- 节能插件 mk1
					["effectivity-module-2"]             = 100   , -- 节能插件 mk2
					["effectivity-module-3"]             = 100   , -- 节能插件 mk3
					["productivity-module"]              = 100   , -- 产能插件 mk1
					["productivity-module-2"]            = 100   , -- 产能插件 mk2
					["productivity-module-3"]            = 100     -- 产能插件 mk3
				}
			}
		}
	} ,
	-- ======================================================================<br>
	-- 添加了一些修改全局基础属性的功能<br>
	-- ======================================================================<br>
	{
		PackageName = "0180_OverhaulBasic" ,
		Requires =
		{
			"0000_Core"
		} ,
		Configs =
		{
			-- ======================================================================<br>
			-- 全局基础修改<br>
			-- 以下是各项设置的默认值<br>
			-- ======================================================================<br>

			-- ======================================================================<br>
			-- 控制修改树木的血量<br>
			-- 填写 true = 开启 , false = 禁用<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			TreeHealth = true ,

			-- ======================================================================<br>
			-- 降低晚上的亮度<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			TotalEvening = true ,

			-- ======================================================================<br>
			-- 敌人单位的尸体存在的持续时间<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			EnemyCorpseDuration = 1200 , -- 20 秒

--			-- ======================================================================<br>
--			-- 现在貌似原版自带了这个功能<br>
--			-- ======================================================================<br>
--			-- 是否显示物品的堆叠信息<br>
--			-- 当一个物品原型数据包含了 localised_description 属性则会使用此属性值作为物品描述 , 否则使用 ["item-description." .. name] 作为物品描述<br>
--			-- ======================================================================<br>
--			-- 只能直接修改<br>
--			-- ======================================================================<br>
--			ShowItemStackData = true ,

--			-- ======================================================================<br>
--			-- 现在貌似原版自带了这个功能<br>
--			-- ======================================================================<br>
--			-- 显示物品堆叠信息时 , 如果物品的数据中没有定义说明信息 , 是否使用一个固定的说明信息替代它<br>
--			-- 如果一个物品没有定义说明信息 , 则在显示时会显示 Unknown Key , 此项设置为 true 会避免这种情况 , 但是可能会掩盖一些物品的说明信息<br>
--			-- 如果没有启用 ShowItemStackData 则此项无效<br>
--			-- ======================================================================<br>
--			-- 只能直接修改<br>
--			-- ======================================================================<br>
--			ShowItemStackDataForce = false ,
		}
	} ,
	-- ======================================================================<br>
	-- 添加了隐藏所有当前不能直接研究的科技的功能<br>
	-- 如果一个科技没有前置科技 , 则它不会被影响<br>
	-- ======================================================================<br>
	{
		PackageName = "0181_HideTechnology" ,
		Requires =
		{
			"0000_Core"
		} ,
		Configs =
		{
			-- ======================================================================<br>
			-- 隐藏科技时仅隐藏本 MOD 或使用本 MOD 的框架创建的科技<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			HideTechnologySelf = true ,

			-- ======================================================================<br>
			-- 隐藏科技的白名单<br>
			-- 位于此名单中的科技不会被隐藏<br>
			-- 仅对本 MOD 或使用本 MOD 的框架创建的科技有效 , 不影响已有存档<br>
			-- 数组 , 填写科技原型的 [ name ] 值 , 作为值的 true 是无效的 , 只是语法优化 , 但是请保持其为 true<br>
			-- 数组本体不可为 nil<br>
			-- 如果没有启用 HideTechnology 则此项无效<br>
			-- ======================================================================<br>
			-- 除了直接修改以外<br>
			-- 也可以在其他 MOD 的 data.lua ( 阶段 ) 中使用代码来修改它们的值<br>
			-- 如果在其他 MOD 中进行了修改 , 那么同时在 control.lua 的 on_init 和 on_load 阶段中也需要进行修改<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Set" , "SIHideTechnology" , "HideTechnologyBlackList" , 值 ) ] 来修改它的值<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Get" , "SIHideTechnology" , "HideTechnologyBlackList" ) ] 来读取它的值<br>
			-- ======================================================================<br>
			HideTechnologyBlackList =
			{
				-- ["科技原型的 [ name ] 值"] = true ,
				-- ...
			}
		}
	} ,
	-- ======================================================================<br>
	-- 添加了 RPG 系统，包括角色属性 , 加点 , 副本 , 装备等一系列逻辑和功能<br>
	-- ======================================================================<br>
	{
		PackageName = "2000_RPGSystem" ,
		Requires =
		{
			"0000_Core"
		} ,
		Configs =
		{
			-- ======================================================================<br>
			-- 世界前缀 , 满足这些前缀的世界进行刷新时 , 会把记录中的旧世界的坐标同步到新世界<br>
			-- 数组 , 填写世界名称的前缀 , 作为值的 true 是无效的 , 只是语法优化 , 但是请保持其为 true<br>
			-- 数组本体不可为 nil<br>
			-- ======================================================================<br>
			-- 除了直接修改以外 , 还可以在其他 MOD 的 control.lua 的 on_init 和 on_load 阶段中<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Set" , "SIRPGSystem" , "SurfacePrefixWhiteList" , 值 ) ] 来修改它的值<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Get" , "SIRPGSystem" , "SurfacePrefixWhiteList" ) ] 来读取它的值<br>
			-- ======================================================================<br>
			SurfacePrefixWhiteList =
			{
				warpzone_ = true
			}
		}
	} ,
	-- ======================================================================<br>
	-- 添加了遗迹系统 , 世界上会出现各种遗迹<br>
	-- ======================================================================<br>
	{
		PackageName = "2100_RuinSystem" ,
		Requires =
		{
			"0000_Core"
		} ,
		Configs =
		{
			-- ======================================================================<br>
			-- 在这个距离内不会生成遗迹<br>
			-- 这是默认值 , 会被世界设置覆盖<br>
			-- ======================================================================<br>
			-- 除了直接修改以外<br>
			-- 也可以在其他 MOD 的 control.lua 的 on_init 和 on_load 阶段中<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Set" , "RuinSystem" , "MinRuinDistance" , 值 ) ] 来修改它的值<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Get" , "RuinSystem" , "MinRuinDistance" ) ] 来读取它的值<br>
			-- ======================================================================<br>
			MinRuinDistance = 1000 ,

			-- ======================================================================<br>
			-- 符合距离时 , 每个区块生成遗迹的概率<br>
			-- 这是默认值 , 会被世界设置覆盖<br>
			-- ======================================================================<br>
			-- 除了直接修改以外<br>
			-- 也可以在其他 MOD 的 control.lua 的 on_init 和 on_load 阶段中<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Set" , "RuinSystem" , "RuinCreateChance" , 值 ) ] 来修改它的值<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Get" , "RuinSystem" , "RuinCreateChance" ) ] 来读取它的值<br>
			-- ======================================================================<br>
			RuinCreateChance = 0.005 ,

			-- ======================================================================<br>
			-- 生成遗迹时 , 遗迹的强度系数<br>
			-- 这是默认值 , 会被世界设置覆盖<br>
			-- ======================================================================<br>
			-- 除了直接修改以外<br>
			-- 也可以在其他 MOD 的 control.lua 的 on_init 和 on_load 阶段中<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Set" , "RuinSystem" , "RuinStrength" , 值 ) ] 来修改它的值<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Get" , "RuinSystem" , "RuinStrength" ) ] 来读取它的值<br>
			-- ======================================================================<br>
			RuinStrength = 1.0 ,

			-- ======================================================================<br>
			-- 生成遗迹时 , 随着与地图中心的距离的增加而获得的额外遗迹强度系数 , 每 32 格增加一次<br>
			-- 这是默认值 , 会被世界设置覆盖<br>
			-- ======================================================================<br>
			-- 除了直接修改以外<br>
			-- 也可以在其他 MOD 的 control.lua 的 on_init 和 on_load 阶段中<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Set" , "RuinSystem" , "RuinStrengthAdden" , 值 ) ] 来修改它的值<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Get" , "RuinSystem" , "RuinStrengthAdden" ) ] 来读取它的值<br>
			-- ======================================================================<br>
			RuinStrengthAdden = 0.146 ,

			-- ======================================================================<br>
			-- 世界前缀 , 满足这些前缀的世界会生成遗迹<br>
			-- 数组 , 填写世界名称的前缀 , 作为值的 true 是无效的 , 只是语法优化 , 但是请保持其为 true<br>
			-- 数组本体不可为 nil<br>
			-- ======================================================================<br>
			-- 除了直接修改以外<br>
			-- 也可以在其他 MOD 的 control.lua 的 on_init 和 on_load 阶段中<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Set" , "RuinSystem" , "SurfacePrefixWhiteList" , 值 ) ] 来修改它的值 , 每次重启游戏都需要设置<br>
			-- 使用代码 [ remote.call( "SIConfigManager" , "Get" , "RuinSystem" , "SurfacePrefixWhiteList" ) ] 来读取它的值<br>
			-- ======================================================================<br>
			SurfacePrefixWhiteList =
			{
				nauvis = true ,
				warpzone_ = true
			}
		}
	} ,
	-- ======================================================================<br>
	-- 添加了更多各种无尽科技<br>
	-- ======================================================================<br>
	{
		PackageName = "3000_InfiniteTechnology" ,
		Requires =
		{
			"0000_Core"
		} ,
		Configs =
		{
			-- ======================================================================<br>
			-- 位于黑名单中的炮塔/武器/地雷不会被加入无尽科技中<br>
			-- 数组 , 填写炮塔/武器/地雷原型的 [ name ] 值 , 作为值的 true 是无效的 , 只是语法优化 , 但是请保持其为 true<br>
			-- 数组本体不可为 nil<br>
			-- ======================================================================<br>
			-- 除了直接修改以外 , 也可以在其他 MOD 的 data.lua ( 阶段 ) 中使用代码来修改它们的值<br>
			-- ======================================================================<br>
			TurretBlackList =
			{
				-- ["炮塔/武器/地雷原型的 [ name ] 值"] = true ,
				-- ...
			}
		}
	} ,
	-- ======================================================================<br>
	-- 添加了高级流体罐装流程<br>
	-- ======================================================================<br>
	{
		PackageName = "3001_AdvancedBarrel" ,
		Requires =
		{
			"0000_Core"
		} ,
		Configs =
		{
			-- ======================================================================<br>
			-- 创建灌装和倾倒配方时 , 是否忽略原版流体的建议装桶设置 [ auto_barrel ]<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			IgnoreAutoBarrel = false ,

			-- ======================================================================<br>
			-- 每个桶可以装的流体数量 , 默认 100<br>
			-- 默认值是参照管道的容量设置的<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			BarrelSize = 100 ,

			-- ======================================================================<br>
			-- 每种桶的物品堆叠数量 , 默认 10<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			BarrelStackSize = 10 ,

			-- ======================================================================<br>
			-- 每种桶的物品标准火箭单次发射数量 , 默认 10<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			BarrelRocketSize = 10 ,

			-- ======================================================================<br>
			-- 禁止装桶的流体的黑名单<br>
			-- 位于此名单中的流体不会创建装桶和倾倒<br>
			-- 数组 , 填写流体原型的 [ name ] 值 , 作为值的 true 是无效的 , 只是语法优化 , 但是请保持其为 true<br>
			-- 数组本体不可为 nil<br>
			-- ======================================================================<br>
			-- 除了直接修改以外 , 也可以在其他 MOD 的 data.lua ( 阶段 ) 中使用代码来修改它们的值<br>
			-- ======================================================================<br>
			BarrelBlackList =
			{
				-- ["流体原型的 [ name ] 值"] = true ,
				-- ...
			}
		}
	} ,
	-- ======================================================================<br>
	-- 添加了红核流程<br>
	-- ======================================================================<br>
	{
		PackageName = "3002_RedCore" ,
		Requires =
		{
			"0000_Core" ,
			"0012_BuildLimit"
		} ,
		Configs =
		{
			-- ======================================================================<br>
			-- 红核流程的难度等级 (其实是减负等级) , 要求是整数 , 取值范围 : 1 至 7 , 等级 1 最难<br>
			-- 前排提示 :<br>
			-- 1. 红核流程不会修改原始游戏流程 (包括配方 , 科技 , 物品属性等) , 但是仍然会给原始游戏难度带来些微的变更<br>
			-- 2. 矿机以外的设备无法放置在红核流程相关矿物上 (包括管道和电线杆等 , 不影响其他矿物)<br>
			-- 3. 如果修改了物品堆叠 , 那么应该避免更改红核流程相关物品的堆叠 , 不然可能会遇到意外的问题 (不兼容所致 , 非反作弊系统引起)<br>
			-- 4. 配方浏览器和量化相关的 MOD 可能会默认给出错误的结果 , 这点还需要注意 (不兼容所致 , 非反作弊系统引起)<br>
			-- 5. 最后 , 红核流程是专门为了难度而制作的 , 致力于追求难度的极限 , 因此各种设定会非常逆天 , 请酌情考虑难度等级<br>
			-- 作为补偿 , 通过红核流程 , 你可以做出来各种神奇的产品<br>
			-- ======================================================================<br>
			-- 各个难度等级的区别 : (相较于上一个难度等级)<br>
			-- 等级 1 : 标准难度 (最难的难度)<br>
			-- 等级 2 : 不再限制使用流程内的插件塔提供加成<br>
			-- 等级 3 : 机器基础能耗 -70% , 机器基础运行速度 +100% , 机器的插件槽 +2<br>
			-- 等级 4 : 原料需求 -70% , 机器的插件槽 +2 , 机器可以使用产能插件和节能插件<br>
			-- 等级 5 : 产物产出 +100% , 移除部分需要循环产物的配方<br>
			-- 等级 6 : 所有拥有概率的产物的概率 +2%<br>
			-- 等级 7 : 配方不再随机分配 (固定为第一套 , 仅影响新存档)<br>
			-- ======================================================================<br>
			-- 只能直接修改<br>
			-- ======================================================================<br>
			DifficultyLevel = 1
		}
	} ,















----	-- ======================================================================<br>
----	-- 书籍逻辑<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0011_Bookshelf"
----	} ,
----	-- ======================================================================<br>
----	-- 补充因为一些原因导致缺失的原版的东西<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0099_BaseFix" ,
----		Requires =
----		{
----			"0000_Core"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了若干矿物<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0101_Resource" ,
----		Configs =
----		{
----			-- ======================================================================<br>
----			-- 处理矿物上无法摆放机器时 , 数字中的实体不受影响<br>
----			-- 数组 , 填写机器原型的 [ name ] 值 , 作为值的 true 是无效的 , 只是语法优化 , 但是请保持其为 true<br>
----			-- 数组本体不可为 nil<br>
----			-- ======================================================================<br>
----			-- 除了直接修改以外 , 也可以在其他 MOD 的 data.lua ( 阶段 ) 中使用代码来修改它们的值<br>
----			-- ======================================================================<br>
----			OreCollisionBlackList =
----			{
----				-- ["机器原型的 [ name ] 值"] = true ,
----				-- ...
----			} ,
----		} ,
----		Requires =
----		{
----			"0000_Core" ,
----			"0012_Unlocker" ,
----			"0013_BuildLimit"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了若干野生植物<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0102_Plant" ,
----		Requires =
----		{
----			"0000_Core" ,
----			"0012_Unlocker" ,
----			"0013_BuildLimit"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了若干野生动物<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0103_Animal" ,
----		Requires =
----		{
----			"0000_Core" ,
----			"0012_Unlocker" ,
----			"0013_BuildLimit"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了种植农业生产的内容<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0201_Planting" ,
----		Requires =
----		{
----			"0101_Resource" ,
----			"0102_Plant" ,
----			"0103_Animal"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了畜牧农业生产的内容<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0202_Husbandry" ,
----		Requires =
----		{
----			"0101_Resource" ,
----			"0102_Plant" ,
----			"0103_Animal"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了蒸汽工业生产的内容<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0211_Steam" ,
----		Requires =
----		{
----			"0101_Resource" ,
----			"0102_Plant" ,
----			"0103_Animal"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了电力工业生产的内容<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0212_Electric" ,
----		Requires =
----		{
----			"0101_Resource" ,
----			"0102_Plant" ,
----			"0103_Animal"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了闪耀工业生产的内容<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0213_Glitter" ,
----		Requires =
----		{
----			"0101_Resource" ,
----			"0102_Plant" ,
----			"0103_Animal"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了焦馏工艺生产的内容<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0221_Scorch" ,
----		Requires =
----		{
----			"0101_Resource" ,
----			"0102_Plant" ,
----			"0103_Animal"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了光辉萃取产的内容<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0222_Light" ,
----		Requires =
----		{
----			"0101_Resource" ,
----			"0102_Plant" ,
----			"0103_Animal"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了暗影铸造产的内容<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0223_Shadow" ,
----		Requires =
----		{
----			"0101_Resource" ,
----			"0102_Plant" ,
----			"0103_Animal"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了子弹物资生产的内容<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0291_Ammo" ,
----		Requires =
----		{
----			"0101_Resource" ,
----			"0102_Plant" ,
----			"0103_Animal"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了防御工事生产的内容<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0292_Defence" ,
----		Requires =
----		{
----			"0101_Resource" ,
----			"0102_Plant" ,
----			"0103_Animal"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了用于种植农业生产的各种设备<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0301_PlantingMachine" ,
----		Requires =
----		{
----			"0201_Planting" ,
----			"0202_Husbandry" ,
----			"0211_Steam" ,
----			"0212_Electric" ,
----			"0213_Glitter" ,
----			"0221_Scorch" ,
----			"0222_Light" ,
----			"0223_Shadow" ,
----			"0291_Ammo" ,
----			"0292_Defence"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了用于畜牧农业生产的各种设备<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0302_HusbandryMachine" ,
----		Requires =
----		{
----			"0201_Planting" ,
----			"0202_Husbandry" ,
----			"0211_Steam" ,
----			"0212_Electric" ,
----			"0213_Glitter" ,
----			"0221_Scorch" ,
----			"0222_Light" ,
----			"0223_Shadow" ,
----			"0291_Ammo" ,
----			"0292_Defence"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了用于蒸汽工业生产的各种设备<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0311_SteamMachine" ,
----		Requires =
----		{
----			"0201_Planting" ,
----			"0202_Husbandry" ,
----			"0211_Steam" ,
----			"0212_Electric" ,
----			"0213_Glitter" ,
----			"0221_Scorch" ,
----			"0222_Light" ,
----			"0223_Shadow" ,
----			"0291_Ammo" ,
----			"0292_Defence"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了用于电力工业生产的各种设备<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0312_ElectricMachine" ,
----		Requires =
----		{
----			"0201_Planting" ,
----			"0202_Husbandry" ,
----			"0211_Steam" ,
----			"0212_Electric" ,
----			"0213_Glitter" ,
----			"0221_Scorch" ,
----			"0222_Light" ,
----			"0223_Shadow" ,
----			"0291_Ammo" ,
----			"0292_Defence"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了用于闪耀工业生产的各种设备<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0313_GlitterMachine" ,
----		Requires =
----		{
----			"0201_Planting" ,
----			"0202_Husbandry" ,
----			"0211_Steam" ,
----			"0212_Electric" ,
----			"0213_Glitter" ,
----			"0221_Scorch" ,
----			"0222_Light" ,
----			"0223_Shadow" ,
----			"0291_Ammo" ,
----			"0292_Defence"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了用于焦馏工艺生产的各种设备<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0321_ScorchMachine" ,
----		Requires =
----		{
----			"0201_Planting" ,
----			"0202_Husbandry" ,
----			"0211_Steam" ,
----			"0212_Electric" ,
----			"0213_Glitter" ,
----			"0221_Scorch" ,
----			"0222_Light" ,
----			"0223_Shadow" ,
----			"0291_Ammo" ,
----			"0292_Defence"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了用于光辉萃取生产的各种设备<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0322_LightMachine" ,
----		Requires =
----		{
----			"0201_Planting" ,
----			"0202_Husbandry" ,
----			"0211_Steam" ,
----			"0212_Electric" ,
----			"0213_Glitter" ,
----			"0221_Scorch" ,
----			"0222_Light" ,
----			"0223_Shadow" ,
----			"0291_Ammo" ,
----			"0292_Defence"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了用于暗影铸造生产的各种设备<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0323_ShadowMachine" ,
----		Requires =
----		{
----			"0201_Planting" ,
----			"0202_Husbandry" ,
----			"0211_Steam" ,
----			"0212_Electric" ,
----			"0213_Glitter" ,
----			"0221_Scorch" ,
----			"0222_Light" ,
----			"0223_Shadow" ,
----			"0291_Ammo" ,
----			"0292_Defence"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了用于子弹物资生产的各种设备<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0391_AmmoMachine" ,
----		Requires =
----		{
----			"0201_Planting" ,
----			"0202_Husbandry" ,
----			"0211_Steam" ,
----			"0212_Electric" ,
----			"0213_Glitter" ,
----			"0221_Scorch" ,
----			"0222_Light" ,
----			"0223_Shadow" ,
----			"0291_Ammo" ,
----			"0292_Defence"
----		}
----	} ,
----	-- ======================================================================<br>
----	-- 增加了用于防御工事生产的各种设备<br>
----	-- ======================================================================<br>
----	{
----		PackageName = "0392_DefenceMachine" ,
----		Requires =
----		{
----			"0201_Planting" ,
----			"0202_Husbandry" ,
----			"0211_Steam" ,
----			"0212_Electric" ,
----			"0213_Glitter" ,
----			"0221_Scorch" ,
----			"0222_Light" ,
----			"0223_Shadow" ,
----			"0291_Ammo" ,
----			"0292_Defence"
----		}
----	} ,
--	-- ======================================================================<br>
--	-- 提供了一些装饰品 , 以及制作它们的方法<br>
--	-- ======================================================================<br>
--	{
--		PackageName = "0500_Colorful" ,
--		Requires =
--		{
--			"0000_Core" ,
--			"0012_Unlocker" ,
--			"0013_BuildLimit"
--		}
--	}
}

return PackageConfig
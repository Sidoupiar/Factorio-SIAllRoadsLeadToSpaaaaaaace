...
local function new_patch_metaset( parameters )
	data.raw["noise-expression"][parameters.count_expression_name] =
	{
		type = "noise-expression" ,
		name = parameters.count_expression_name ,
		expression = tne( 0 )
	}
	return
	{
		count_expression_name = parameters.count_expression_name ,
...
local resource_autoplace__patch_metasets = nil
...
-- - HasStarting - true|false|nil - yes, no, and there is no special starting area, respectively
...
function SIAutoPlace.Create( parameters )
	...
	local autoplace_control_name = parameters.autoplace_control_name or name

	-- How much of this stuff (probability * richness) should occur per tile on average near the starting area?
	local base_density = parameters.BaseDensity
	...
	local random_probability = parameters.random_probability or 1
	local base_spots_per_km2 = parameters.base_spots_per_km2 or 2.5
	local random_spot_size_minimum = parameters.random_spot_size_minimum or 0.25
	local random_spot_size_maximum = parameters.random_spot_size_maximum or 2.00
	-- Amplitude of spot 'blob noise' relative to typical spot amplitude
	local regular_blob_amplitude_multiplier = 1/8 * ( parameters.regular_blob_amplitude_multiplier or 1 )
	local starting_blob_amplitude_multiplier = 1/8 * ( parameters.starting_blob_amplitude_multiplier or 1 )
	...
	local additional_richness = parameters.additional_richness or 0
	...
	local minimum_richness = parameters.minimum_richness or 0
	-- 'post' as in multiplied after everything else is calculated, including additional_richness
	-- and minimum_richness.
	local richness_post_multiplier = ( parameters.richness_post_multiplier or 1 ) * control_setting.richness_multiplier

	local seed1 = parameters.seed1 or 100
	...
	local regular_rq_factor = ( parameters.RegularMultiplier or 1 ) * 1 / 10
	local starting_rq_factor = ( parameters.StartingMultiplier or 1 ) * 1 / 7
	...
	if parameters.HasStarting == nil then
	...
	local function regular_density_at( dist )
		...
		local effective_distance = noise.clamp( size_effective_distance_at( dist ) , 0 , spot_enlargement_maximum_distance )
	...
	local candidate_spot_count = parameters.candidate_spot_count or 21
	...
	local starting_modulation = noise.clamp( ( starting_resource_placement_radius - distance ) * starting_area_sharpness , 0 , 1 )
	...
	if parameters.HasStarting == true then
	...
	local starting_spots = tne
	{
		...
		arguments =
		{
			...
			spot_favorability_expression = litexp( starting_feasibility * 2 - 1 * distance / starting_resource_placement_radius + noise.random( 0.5 ) ) ,
			basement_value = basement_value ,
	...
	elseif parameters.HasStarting == false then
